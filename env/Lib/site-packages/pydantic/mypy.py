...
class PydanticModelTransformer:
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901
        ...
        """Get pydantic models field from statement.

        Args:
            stmt: The statement.
            model_config: Configuration settings for the models.
            class_vars: ClassVars already known to be defined on the models.

        Returns:
            A pydantic models field if it could find the field in statement. Otherwise, `None`.
        """
        cls = self._cls
    ...
    def is_field_frozen(stmt: AssignmentStmt) -> bool:
        ...
        sense; this does not determine whether the field is frozen because the entire models is frozen; that is
        handled separately.
        """
        expr = stmt.rvalue
        if isinstance(expr, TempNode):
...
class ModelConfigData:
    """Pydantic mypy plugin models config class."""
    ...
    def get_values_dict(self) -> dict[str, Any]:
        """Returns a dict of Pydantic models config names to their values.
    ...
    def update(self, config: ModelConfigData | None) -> None:
        """Update Pydantic models config values."""
        if config is None:
            return
    ...
    def setdefault(self, key: str, value: Any) -> None:
        """Set default value for Pydantic models config if config value is `None`."""
...
def error_from_attributes(model_name: str, api: CheckerPluginInterface, context: Context) -> None:
    """Emits an error when the models does not have `from_attributes=True`."""
...
def error_untyped_fields(api: SemanticAnalyzerPluginInterface, context: Context) -> None:
    """Emits an error when there is an untyped field in the models."""
