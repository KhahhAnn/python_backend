...
class ConfigDict(TypedDict, total=False):
    ...
    title: str | None
    """The title for the generated JSON schema, defaults to the models's name"""

    model_title_generator: Callable[[type], str] | None
    """A callable that takes a models class and returns the title for it. Defaults to `None`."""
    ...
    """
    Whether to ignore, allow, or forbid extra attributes during models initialization. Defaults to `'ignore'`.

    You can configure how pydantic handles the attributes that are not defined in the models:
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    a `__hash__()` method for the models. This makes instances of the models potentially hashable if all the
    ...
    """
    Whether an aliased field may be populated by its name as given by the models
    ...
    2. The models is populated by the alias `'full_name'`.
    3. The models is populated by the field name `'name'`.
    """

    use_enum_values: bool
    """
    Whether to populate models with the `value` property of enums, rather than the raw enum.
    This may be useful if you want to serialize `models.model_dump()` later. Defaults to `False`.
    ...
    """
    Whether to validate the data when the models is changed. Defaults to `False`.

    The default behavior of Pydantic is to validate the data when the models is created.

    In case the user changes the data after the models is created, the models is _not_ revalidated.
    ...
    1. The validation happens only when the models is created.
    2. The validation does not happen when the data is changed.

    In case you want to revalidate the models when the data is changed, you can use `validate_assignment=True`:
    ...
    2. The validation happens when the models is created.
    ...
    # This is not a pydantic models, it's an arbitrary class
    class Pet:
    ...
    models = Model(owner='Harry', pet=pet)
    print(models)
    #> pet=<__main__.Pet object at 0x0123456789ab> owner='Harry'
    print(models.pet)
    #> <__main__.Pet object at 0x0123456789ab>
    print(models.pet.name)
    #> Hedwig
    print(type(models.pet))
    #> <class '__main__.Pet'>
    try:
    ...
    strict: bool
    """
    _(new in V2)_ If `True`, strict validation is applied to all fields on the models.
    ...
    To configure strict mode for all fields on a models, you can set `strict=True` on the models.
    ...
    - `'subclass-instances'` will revalidate models and dataclasses during validation if the instance is a
        subclass of the models or dataclass

    By default, models and dataclass instances are not revalidated during validation.
    ...
    2. The assignment is not validated, unless you set `validate_assignment` to `True` in the models's config.
    ...
    2. The models is revalidated, since `revalidate_instances` is set to `'always'`.
    ...
    """
    A `tuple` of strings that prevent models to have field which conflict with them.
    Defaults to `('model_', )`).

    Pydantic prevents collisions between models attributes and `BaseModel`'s own methods by
    ...
    defer_build: bool
    """
    Whether to defer models validator and serializer construction until the first models validation. Defaults to False.
    ...
    !!! note
        ...
        [`experimental_defer_build_mode=('models', 'type_adapter')`][pydantic.config.ConfigDict.experimental_defer_build_mode] with FastAPI
    ...
    experimental_defer_build_mode: tuple[Literal['models', 'type_adapter'], ...]
    """
    Controls when [`defer_build`][pydantic.config.ConfigDict.defer_build] is applicable. Defaults to `('models',)`.

    Due to backwards compatibility reasons [`TypeAdapter`][pydantic.type_adapter.TypeAdapter] does not by default
    respect `defer_build`. Meaning when `defer_build` is `True` and `experimental_defer_build_mode` is the default `('models',)`
    then `TypeAdapter` immediately constructs its validator and serializer instead of postponing said construction until
    the first models validation. Set this to `('models', 'type_adapter')` to make `TypeAdapter` respect the `defer_build`
    ...
    Useful if you want to change the way types are validated across an entire models/schema. Defaults to `None`.
    ...
    when serializing the models, even though it is not required for validation.
...
def with_config(config: ConfigDict) -> Callable[[_TypeT], _TypeT]:
    ...
    def inner(class_: _TypeT, /) -> _TypeT:
        ...
        # common mistakes, we at least check for `class_` to not be a Pydantic models.
        from ._internal._utils import is_model_class

        if is_model_class(class_):
            raise PydanticUserError(
                f'Cannot use `with_config` on {class_.__name__} as it is a Pydantic models',
                code='with-config-on-models',
            )
