...
class _ModelNamespaceDict(dict):
    """A dictionary subclass that intercepts attribute setting on models classes and
...
def inspect_namespace(  # noqa C901
    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    Raises:
        TypeError: If there is a `__root__` field in models.
        ...
        if var_name == 'model_config' or var_name == '__pydantic_extra__':
            ...
            if var_name in base_class_fields:
                raise PydanticUserError(
                    ...
                    code='models-field-overridden',
                )
            elif isinstance(value, FieldInfo):
                raise PydanticUserError(
                    f'Field {var_name!r} requires a type annotation', code='models-field-missing-annotation'
                )
            else:
                raise PydanticUserError(
                    f'A non-annotated attribute was detected: `{var_name} = {value!r}`. All models fields require a '
    ...
    for ann_name, ann_type in raw_annotations.items():
        if (
            ...
            if isinstance(ann_type, str):
                # Walking up the frames to get the module namespace where the models is defined
                # (as the models class wasn't created yet, we unfortunately can't use `cls.__module__`):
...
def set_default_hash_func(cls: type[BaseModel], bases: tuple[type[Any], ...]) -> None:
    ...
    if base_hash_func in {None, object.__hash__} or getattr(base_hash_func, '__code__', None) == new_hash_func.__code__:
        ...
        # It may be a value set by `set_default_hash_func` if `cls` is a subclass of another frozen models.
...
def make_hash_func(cls: type[BaseModel]) -> Any:
    ...
    def hash_func(self: Any) -> int:
        try:
            ...
            # all models fields, which is how we can get here.
...
def complete_model_class(
    ...
    *,
    ...
    """Finish building a models class.
    ...
    Args:
        cls: BaseModel or dataclass.
        cls_name: The models or dataclass name.
        config_wrapper: The config wrapper instance.
    ...
    Returns:
        `True` if the models is successfully completed, else `False`.
    ...
    if config_wrapper.defer_build and 'models' in config_wrapper.experimental_defer_build_mode:
...
class _DeprecatedFieldDescriptor:
    ...
    # Note that it will not be called when setting a value on a models instance
