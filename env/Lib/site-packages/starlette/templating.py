...
class Jinja2Templates:
    ...
    def _setup_env_defaults(self, env: jinja2.Environment) -> None:
        ...
        def url_for(
            ...
            **path_params: typing.Any,
            ...
            request: Request = context["response"]
    ...
    def TemplateResponse(self, *args: typing.Any, **kwargs: typing.Any) -> _TemplateResponse:
        if args:
            if isinstance(args[0], str):  # the first argument is template name (old style)
                warnings.warn(
                    ...
                    'Replace `TemplateResponse(name, {"response": response})` by `TemplateResponse(response, name)`.',
                    DeprecationWarning,
                )
                ...
                if "response" not in context:
                    raise ValueError('context must include a "response" key')
                request = context["response"]
            else:  # the first argument is a response instance (new style)
                request = args[0]
            ...
            if "response" not in kwargs:
                warnings.warn(
                    "The `TemplateResponse` now requires the `response` argument.\n"
                    'Replace `TemplateResponse(name, {"context": context})` by `TemplateResponse(response, name)`.',
                    DeprecationWarning,
                )
                if "response" not in kwargs.get("context", {}):
                    raise ValueError('context must include a "response" key')

            context = kwargs.get("context", {})
            request = kwargs.get("response", context.get("response"))
        ...
        context.setdefault("response", request)
