...
def test_max_incomplete_event_size_countermeasure() -> None:
    ...
    # We can respond and reuse to get the second pipelined response
    c.send(Response(status_code=200, headers=[]))  # type: ignore[arg-type]
    c.send(EndOfMessage())
    c.start_next_cycle()
    assert get_all_events(c) == [
...
def test_protocol_switch() -> None:
    for (req, deny, accept) in [
        ...
        def setup() -> ConnectionPair:
            ...
            # finish the response before that kicks in
            for conn in p.conns:
...
def test_close_different_states() -> None:
    ...
    # Server after response -> not allowed
    p = ConnectionPair()
    p.send(CLIENT, req)
    with pytest.raises(LocalProtocolError):
...
def test_errors() -> None:
    ...
    for role in [CLIENT, SERVER]:
        if role is CLIENT:
            # This HTTP/1.0 response won't be detected as bad until after we go
