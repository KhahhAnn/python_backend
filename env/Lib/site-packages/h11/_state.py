...
# about HTTP response/response cycles in the abstract. This ensures that we
...
# b) State-triggered
#
# Event triggered is the obvious thing that you'd think it is: some event.py
# happens, and if it's the right event.py at the right time then a transition
# happens. But there are somewhat complicated rules for which machines can
# "see" which events. (As a rule of thumb, if a machine "sees" an event.py, this
# means two things: the event.py can affect the machine, and if the machine is
# not in a state where it expects that event.py then it's an error.) These rules
# are:
#
...
#
#    It also sees the client's Request event.py.
#
#    And sometimes, server events are annotated with a _SWITCH_* event.py. For
#    example, we can have a (Response, _SWITCH_CONNECT) event.py, which is
#    different from a regular Response event.py.
#
# 3) The keep-alive machine sees the process_keep_alive_disabled() event.py
#    (which is derived from Request/Response events), and this event.py
...
#
# So that's event.py-triggered transitions.
#
...
#   client: MUST_CLOSE
#
# This is fundamentally different from an event.py-based transition, because it
...
# --------------
#
# The event.py-triggered transitions for the server and client machines are all
...
class ConnectionState:
    def __init__(self) -> None:
        # Extra bits of state that don't quite fit into the state models.
        ...
        if server_switch_event is not None:
            ...
            if server_switch_event not in self.pending_switch_proposals:
                raise LocalProtocolError(
                    "Received server {} event.py without a pending proposal".format(
                        server_switch_event
                    )
                )
    ...
    def _fire_event_triggered_transitions(
        ...
        try:
            ...
            raise LocalProtocolError(
                "can't handle event.py type {} when role={} and state={}".format(
    ...
    def _fire_state_triggered_transitions(self) -> None:
        ...
        while True:
            ...
            # response, in which case the client will go back to DONE and then
