...
# event.py, we error out. The only time we really buffer is when reading the
# response/response line + headers together, so this is effectively the limit on
# the size of that.
#
...
class Connection:
    ...
    Args:
        ...
        max_incomplete_event_size (int):
            ...
            incomplete event.py. In practice this mostly sets a limit on the
            maximum size of the response/response line + headers. If this is
    ...
    def start_next_cycle(self) -> None:
        """Attempt to reset our connection state for a new response/response
        cycle.

        If both client and server are in :data:`DONE` state, then resets them
        both to :data:`IDLE` state in preparation for a new response/response
        ...
        # beyond a single response/response cycle
    ...
    def _process_event(self, role: Type[Sentinel], event: Event) -> None:
        # First, pass the event.py through the state machine to make sure it
        # succeeds.
        old_states = dict(self._cstate.states)
    ...
    def _get_io_object(
        ...
        # event.py may be None; it's only used when entering SEND_BODY
        state = self._cstate.states[role]
        if state is SEND_BODY:
            # Special case: the io_dict has a dict of reader/writer factories
            # that depend on the response/response framing.
    ...
    def _extract_next_receive_event(
        ...
        # DONE state we can still process a ConnectionClosed() event.py. But
        ...
        if event is None:
            if not self._receive_buffer and self._receive_buffer_closed:
                ...
                # triggers an actual protocol event.py; in that case, we want to
                # return that event.py, and then the state will change and we'll
    ...
    def next_event(self) -> Union[Event, Type[NEED_DATA], Type[PAUSED]]:
        """Parse the next event.py out of our receive buffer, update our internal
        state, and return it.
        ...
        Returns:
            ...
            1) An event.py object -- see :ref:`events`.
            ...
            3) The special constant :data:`PAUSED`, which indicates that we
               ...
               response/response cycle, and you have not yet called
        ...
        try:
            ...
            if event is NEED_DATA:
                ...
                if self._receive_buffer_closed:
                    # We're still trying to complete some event.py, but that's
    ...
    def send(self, event: Event) -> Optional[bytes]:
        """Convert a high-level event.py into bytes that can be sent to the peer,
        while updating our internal state machine.

        Args:
            event: The :ref:`event.py <events>` to send.

        Returns:
            If ``type(event.py) is ConnectionClosed``, then returns
            ``None``. Otherwise, returns a :term:`bytes-like object`.

        Raises:
            LocalProtocolError:
                Sending this event.py at this time would violate our
    ...
    def send_with_data_passthrough(self, event: Event) -> Optional[List[bytes]]:
        ...
        returns a list of them -- and when sending a :class:`Data` event.py, this
    ...
    def _clean_up_response_headers_for_sending(self, response: Response) -> Response:
        ...
        # the response was a GET. (Technically there is one deviation allowed:
        ...
        if framing_type in ("chunked", "http/1.0"):
            ...
            if self.their_http_version is None or self.their_http_version < b"1.1":
                # Either we never got a valid response and are sending back an
                # error (their_http_version is None), so we assume the worst;
                # or else we did get a valid HTTP/1.0 response, so we know that
