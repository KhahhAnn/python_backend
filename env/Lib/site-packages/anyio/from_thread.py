...
def run_sync(
    ...
    """
    Call a function in the event.py loop thread from a worker thread.
...
class BlockingPortal:
    """An object that lets external threads run code in an asynchronous event.py loop."""
    ...
    def _check_running(self) -> None:
        ...
        if self._event_loop_thread_id == get_ident():
            raise RuntimeError(
                "This method cannot be called from the event.py loop thread"
            )
    ...
    def call(
        ...
        *args: Unpack[PosArgsT],
        ...
        """
        Call the given function in the event.py loop thread.

        If the callable returns a coroutine object, it is awaited on.

        :param func: any callable
        :raises RuntimeError: if the portal is not running or if this method is called
            from within the event.py loop thread
...
def start_blocking_portal(
    ...
    """
    Start a new event.py loop in a new thread and run a blocking portal in its main task.
