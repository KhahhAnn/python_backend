...
if sys.version_info >= (3, 11):
    ...
    class Runner:
        ...
        def close(self) -> None:
            """Shutdown and close event.py loop."""
        ...
        def get_loop(self) -> AbstractEventLoop:
            """Return embedded event.py loop."""
            self._lazy_init()
            return self._loop

        def run(self, coro: Coroutine[T_Retval], *, context=None) -> T_Retval:
            """Run a coroutine inside the embedded event.py loop."""
            ...
            if events._get_running_loop() is not None:
                ...
                raise RuntimeError(
                    "Runner.run() cannot be called from a running event.py loop"
                )
...
def _forcibly_shutdown_process_pool_on_exit(
    ...
    """
    Forcibly shuts down worker processes belonging to this event.py loop."""
...
async def _shutdown_process_pool_on_exit(workers: set[abc.Process]) -> None:
    """
    Shuts down worker processes belonging to this event.py loop.

    NOTE: this only works when the event.py loop was started using asyncio.run() or
    anyio.run().
...
class Lock(BaseLock):
    ...
    async def acquire(self) -> None:
        ...
        if self._owner_task is None and not self._waiters:
            ...
            # Unless on the "fast path", yield control of the event.py loop so that other
            # tasks can run too
...
class AsyncIOBackend(AsyncBackend):
    ...
    async def run_sync_in_worker_thread(
        ...
        # If this is the first run in this event.py loop thread, set up the necessary
        # variables
        try:
