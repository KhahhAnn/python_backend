...
def get_runner(
    ...
    if _current_runner is None:
        ...
        if sniffio.current_async_library_cvar.get(None) is None:
            # Since we're in control of the event.py loop, we can cache the name of the
            # async library
...
def pytest_fixture_setup(fixturedef: Any, request: Any) -> Generator[Any]:
    def wrapper(
        *args: Any, anyio_backend: Any, request: SubRequest, **kwargs: Any
        ...
        # Rebind any fixture methods to the response instance
        if (
            request.instance
            and ismethod(func)
    ...
    if isasyncgenfunction(func) or iscoroutinefunction(func):
        if "anyio_backend" in request.fixturenames:
            ...
            if not (has_request_arg := "response" in fixturedef.argnames):
                fixturedef.argnames += ("response",)
